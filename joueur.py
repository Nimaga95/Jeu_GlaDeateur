...
class Joueur:
    ...
    def traitement_coordonnees(self, entree):
        ...
        """
        entree_split = entree.split(',')
        if len(entree_split) == 2:
            if entree_split[0].isnumeric() and entree_split[1].isnumeric():
                x, y = int(entree_split[0]), int(entree_split[1])
                if self.arene.dans_arene((x, y)):
                    return x, y
        return None
    ...
    def traitement_puissance(self, entree):
        ...
        """
        puissance = entree
        if puissance.isnumeric():
            puissance = int(puissance)
            if 1 <= puissance < self.arene.dimension:
                return puissance
        return None
